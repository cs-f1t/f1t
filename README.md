...
운영 DB에는 실제 랭킹에 사용하는 Gemini 이미지·소재 임베딩만 유지합니다. 실험용 CLIP/Qwen 벡터는 운영 테이블에서 분리해 중복 저장으로 인한 용량 증가를 막습니다. 세부 운영 기준과 정리 절차는 [`pipeline/README.md`](pipeline/README.md#운영-db-용량-정책)에 정리했습니다.
...
```mermaid
flowchart LR
    A["사용자 요청<br/>자연어 · 선택적 참고 이미지"]
    B["의도·속성 추출"]
    C["Target Description 생성"]
    D["검색 문장<br/>768차원 임베딩"]
    E["선택: 소재 요청<br/>768차원 임베딩"]
    F{"정확한 속성<br/>조건이 있는가?"}

    subgraph META["메타데이터 우선 경로"]
        direction TB
        G["Supabase REST<br/>메타데이터 후보 축소"]
        H{"후보를<br/>확보했는가?"}
        I["저장된 벡터로<br/>코사인 유사도 계산"]
        G --> H
        H -- 성공 --> I
    end

    subgraph VECTOR["벡터 검색 경로"]
        direction TB
        J["Supabase 벡터 검색 RPC"]
        K["이미지 HNSW<br/>후보 50~100개 탐색"]
        L["후보별 이미지·소재<br/>코사인 유사도 계산"]
        J --> K --> L
    end

    M{"소재 조건이<br/>있는가?"}
    N["이미지 유사도 100%"]
    O["이미지 0.8 + 소재 0.2<br/>가중합 재정렬"]

    subgraph RESULT["결과 제공"]
        direction TB
        P["속성 후처리 · 결과 정리"]
        Q["최종 Top-K 상품"]
        R["한국어 추천 이유 생성"]
...
의도 추출과 Target Description 생성을 병렬로 수행합니다. 정확한 속성 조건이 있으면 메타데이터로 후보를 먼저 축소하고 애플리케이션에서 유사도를 계산합니다. 후보를 확보하지 못했거나 일반 의미 검색이 필요한 경우에는 이미지 HNSW로 후보를 탐색합니다. 두 경로 모두 소재 조건이 있을 때만 이미지 0.8·소재 0.2 가중합으로 재정렬합니다.
...
추출한 의도를 상품 DB의 카테고리별 Supabase 테이블과 연결합니다. 카테고리로 대상 테이블을 선택하고 소매, 기장, 성별, 계절, 신축성, 두께, 핏처럼 정확히 비교할 수 있는 속성으로 후보를 먼저 줄입니다. 색상과 소재는 이 단계에서 강제 필터링하지 않고 의미 검색과 가중 랭킹에 반영합니다.
...
| 벡터 검색 | 정확한 후보가 없거나 메타데이터 조회가 실패함 | Supabase 벡터 검색 RPC가 이미지 HNSW 후보를 찾고 최종 점수를 계산 |
